...
@objc public final class JMCallKitProxy: NSObject {
    ...
    @objc public static var muidOne2oneDic = [String : Bool]()
...
//            _ = try? AVAudioSession.sharedInstance().setCategory(.playAndRecord, options: .mixWithOthers)
//            _ = try? AVAudioSession.sharedInstance().overrideOutputAudioPort(AVAudioSession.PortOverride.none)
//            if hasVideo == true {
//                _ = try? AVAudioSession.sharedInstance().setMode(AVAudioSession.Mode.videoChat)
//            } else {
...
//            }
//        }
        if muidOne2oneDic.keys.first?.lowercased() == UUID.uuidString.lowercased(), muidOne2oneDic.values.first == true{
            return
        }
