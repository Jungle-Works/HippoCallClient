...
class CallClient{
    ...
    func startNew(call: Call, completion: @escaping (Bool) -> Void) {
        startNew(call: call) { success, _ in completion(success) }
    }
    ...
    func loadCallPresenterView(uuid: String = "", defaultType: Call.CallType = .audio) {
        ...
        self.callPresenter =  HippoCallClient.shared.delegate?.loadCallPresenterView(request: request)
    ...
    private func readyToConnectReceived(signal: CallSignal) {
        ...
        activeCall?.rtcClient?.startNewCall(completion: { _ in })
    }
    ...
    fileprivate func connectionLost() {
        callPresenter?.callFailedDueToNetworkError()
        callDisconnected()
    }
